...
extension VpnManager{
    ...
    fileprivate func createProviderManager() -> NETunnelProviderManager {
        ...
        conf.serverAddress = "HistoryTagVPN"
        self.vpnManager.protocolConfiguration = conf
        self.vpnManager.localizedDescription = "HistoryTagVPN VPN"
        return self.vpnManager
    }
    
    
    func loadAndCreatePrividerManager(config:VPNModel ,_ complete: @escaping (NETunnelProviderManager?) -> Void ){
        NETunnelProviderManager.loadAllFromPreferences{ (managers, error) in
            ...
            self.setRulerConfig(config: config,manager)
//            self.loadConfig(config: config, manager)
...
extension VpnManager{
    ...
    func connect(config:VPNModel){
        self.loadAndCreatePrividerManager(config:config) { (manager) in
...
extension VpnManager{
    ...
    fileprivate func setRulerConfig(config:VPNModel,_ manager:NETunnelProviderManager){
        var conf = [String:AnyObject]()
        
//        conf["ss_address"] = "173.82.255.234" as AnyObject?
//        conf["ss_port"] = 517 as AnyObject?
//        conf["ss_method"] = "CHACHA20" as AnyObject?
//        conf["ss_password"] = "qwert123" as AnyObject?
//        conf["ymal_conf"] = getRuleConf() as AnyObject?
        conf["ss_address"] = config.ss_address as AnyObject?
        conf["ss_port"] = config.ss_port  as AnyObject?
        conf["ss_method"] = config.ss_method as AnyObject?
        conf["ss_password"] = config.ss_password as AnyObject?
        conf["ymal_conf"] = config.ymal_conf as AnyObject?
        let orignConf = manager.protocolConfiguration as! NETunnelProviderProtocol
        orignConf.providerConfiguration = conf
        manager.protocolConfiguration = orignConf
    }

   fileprivate func loadConfig(config:VPNModel,_ manager:NETunnelProviderManager) {
        var conf = [String:AnyObject]()
        conf["ss_address"] = config.ss_address as AnyObject?
        conf["ss_port"] = config.ss_port  as AnyObject?
        conf["ss_method"] = config.ss_method as AnyObject?
